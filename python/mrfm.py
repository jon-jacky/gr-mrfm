# mrfm.py, based on mrfm.py in GNU Radio
# 
...
class source_c(usrp.source_c):
    def __init__(self,fpga_filename):
        ...
        # ADC A and B
        self._write_9862(0,2,0x80)  # Rx A, Bypass ADC buffer, minimum gain
        self._write_9862(0,3,0x80)  # Rx B, Bypass ADC buffer, minimum gain

        # DAC A only, TX is for both 
        self._write_9862(0,8,0)   # TX PWR Down
        self._write_9862(0,10,0)  # DAC A offset [1:0]
        self._write_9862(0,11,0)  # DAC A offset [9:2]
        self._write_9862(0,14,0x80)  # DAC A gain
        self._write_9862(0,16,0xff)  # TX PGA gain
        self._write_9862(0,18,0x0c)  # TX IF
        self._write_9862(0,19,0x01)  # TX Digital
        self._write_9862(0,20,0x00)  # TX Mod

        # Repeat for B
        self._write_9862(0,12,0)  # DAC B offset [1:0]
        self._write_9862(0,13,0)  # DAC B offset [9:2]
        self._write_9862(0,15,0x80)  # DAC B gain
    ...
    # Uses frac_bits (not coeff_frac_bits) in data path, to configure shifter
    def set_coeffs(self,frac_bits,b20,b10,b00,a20,a10,b21,b11,b01,a21,a11):

        wordlength = frac_bits + 2      # use two bits for sign, magnitude
        wordbitsset = 2**wordlength - 1  # lsb's all set, up through wordlength

        def make_val(address,value):
            # return (address << 16) | (value & 0xffff)
            # return (address << 24) | (value & 0xffffff)
            return (address << wordlength) | (value & wordbitsset)
    ...
    def set_scales(self, scale0, scale1):
        self._write_fpga_reg(75, scale0)  # reg 75, 76 defined in mrfm.vh
        self._write_fpga_reg(76, scale1)        
